...
ResourceManager::ResourceManager(const ResourceManagerInfo& info)
    : prefixes{.shaders_path_prefix = info.shaders_path_prefix,
      ...
      transfer_storage(Engine::GetInstance()->GetRenderEngine(), info.transfer_storage_info)
{
...
{
    auto res_path = (prefixes.shaders_path_prefix / path).lexically_normal();
    auto res_path_string = res_path.string();

    auto it = resources.shaders.find(res_path_string);
    ...
    auto ins_it = resources.shaders.insert(std::pair{res_path_string, shader_ptr});
...
{
    auto res_path = (prefixes.images_path_prefix / path).lexically_normal();
    auto res_path_string = res_path.string();

    auto it = resources.images.find(res_path_string);
    ...
    auto ins_it = resources.images.insert(std::pair{res_path_string, image});
    ...
    transfer_storage.Transfer(TransferCallbackOperation{
        .cback = [img_entry = ins_it.first->second]([[maybe_unused]] Render::CommandBuffer* cmd)
        {
            img_entry->SetReady();
        }});
