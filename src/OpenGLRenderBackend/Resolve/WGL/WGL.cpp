...
namespace OpenGL
{
    ...
    static Render::ContextProperties get_context_properties()
    {
        ...
        Render::ContextLimits limits = Render::ContextLimits{
        ...
        Render::ContextFeatures features = Render::ContextFeatures{
        ...
        return Render::ContextProperties{
            .context_name = "OpenGL",
    ...
    static LRESULT CALLBACK DummyWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        switch(uMsg)
        {
            case WM_CREATE:
            {
                ...
                if(format_index == 0)
                {
                    *window_params_exp = get_last_error();
                    return -1;
                }
                ...
                */
                ...
                std::vector<std::uint32_t> swapchain_config_indices;
                ...
                Render::ContextSurfaceCapabilities surface_capabilities =
                    Render::ContextSurfaceCapabilities{
                        .min_image_count = 0,
                        .max_image_count = 2,
                ...
                *window_params_exp =
                    WindowParams{.ctx_init_properties = {.properties = get_context_properties(),
                                                         .surface_capabilities =
                                                             std::move(surface_capabilities)},
                                 .pixelformat_indices = std::move(swapchain_config_indices)};
                break;
            }
        }
    ...
    WGL::WGL(const Render::ResolveWin32Info& _info)
        : info(_info),
          glrc(nullptr)
    {
        ...
        hrs::scoped_call cleanup(
            [&_window, hinstance = info.instance]()
            {
                if(_window != 0)
        ...
        this->ctx_init_properties = std::move(window_param_exp.value().ctx_init_properties);
        this->pixelformat_indices = std::move(window_param_exp.value().pixelformat_indices);
    }
    ...
    const Render::ContextInitProperties& WGL::GetInitProperties() const noexcept
    {
        return ctx_init_properties;
    }

    Context* WGL::CreateContext(const Render::SelectedContextDesc& desc)
    {
        if(glrc)
        ...
        DescribePixelFormat(info.hdc,
                            pixelformat_indices[desc.selected_config_index],
                            sizeof(PIXELFORMATDESCRIPTOR),
                            &pfd);

        if(SetPixelFormat(info.hdc, pixelformat_indices[desc.selected_config_index], &pfd) == FALSE)
        ...
        if(!desc.enabled_features.validation_layer)
            profile_attributes[8] = WGL_CONTEXT_OPENGL_NO_ERROR_ARB;

        if(desc.enabled_features.robust_buffer_access)
            profile_attributes[5] |= WGL_CONTEXT_ROBUST_ACCESS_BIT_ARB;

        glrc = glad_wglCreateContextAttribsARB(info.hdc, nullptr, profile_attributes);
        if(!glrc)
            throw get_last_error();

        return new Context(
            ctx_init_properties.properties,
            Render::ContextSurfaceCapabilities{
                .min_image_count = ctx_init_properties.surface_capabilities.min_image_count,
                .max_image_count = ctx_init_properties.surface_capabilities.max_image_count,
                .supported_present_modes =
                    ctx_init_properties.surface_capabilities.supported_present_modes,
                .supported_configs = {ctx_init_properties.surface_capabilities
                                          .supported_configs[desc.selected_config_index]}});
    }
};