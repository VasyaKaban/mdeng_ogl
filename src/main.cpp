...
layout(location = 2) in vec3 input_normals;
...
void main()
{
    ...
    output_color = vec4(input_normals, 1.0);
}
)";
...
constexpr auto BLIT_VERTEX_SHADER =
    R"(
#version 460 core

layout(location = 0) out vec2 output_texture_coords;

const vec2 SCREEN_RECT_POSITIONS[6] = {
    vec2(-1.0, 1.0),
    vec2(1.0, 1.0),
    vec2(-1.0, -1.0),
...
const vec2 SCREEN_RECT_TEXTURE_COORDINATES[6] = {
    vec2(0, 1000),
    vec2(1600, 1000),
    vec2(0, 0),
...
void main()
{
    gl_Position = vec4(SCREEN_RECT_POSITIONS[gl_VertexID], 0.0, 1.0);
    output_texture_coords = SCREEN_RECT_TEXTURE_COORDINATES[gl_VertexID];
}
)";

constexpr auto BLIT_FRAGMENT_SHADER =
    R"(
#version 460 core
...
layout(binding = 0) uniform sampler2DMS input_attachment;

void main()
{
    vec4 color = vec4(0.0, 0.0, 0.0, 0.0);
    for(int i = 0; i < 4; i++)
        color += texelFetch(input_attachment, ivec2(input_texture_coords), i);

    output_color = color / 4;
}
)";

constexpr auto CUBEMAP_VERTEX_SHADER =
    R"(
#version 460 core

layout(location = 0) out vec3 output_texture_coordinates;
...
void main()
{
    gl_Position = projection * view * vec4(CUBE_POSITIONS[CUBE_INDICES[gl_VertexID]], 1.0);
    output_texture_coordinates = CUBE_COORDS[CUBE_INDICES[gl_VertexID]];
}
)";

constexpr auto CUBEMAP_FRAGMENT_SHADER =
    R"(
#version 460 core

layout(location = 0) in vec3 iutput_texture_coordinates;

layout(location = 0) out vec4 output_color;

layout(binding = 0) uniform samplerCube first_cubemap;
layout(binding = 1) uniform samplerCube second_cubemap;

layout(location = 0) uniform float lerp_t;

void main()
{
    output_color = mix(texture(first_cubemap, iutput_texture_coordinates), texture(second_cubemap, iutput_texture_coordinates), lerp_t);
}
)";
...
#pragma message("HERE!")
        ...
        for(const auto& filename: filenames)
        {
            ...
            ifs.open(
                std::format(
                    R"(D:\steam\steamapps\common\Stalker Call of Pripyat\UEgamedata\textures\sky\{})",
                    filename),
        ...
        Pipeline pipeline;
        {
            const ShaderInfo vertex_shader_info = {
                ...
                .code = VERTEX_SHADER,
                .code_size = static_cast<GLint>(std::strlen(VERTEX_SHADER))};
            Shader vertex_shader(ctx, vertex_shader_info);

            const ShaderInfo fragment_shader_info = {
                .stage = ShaderStage::Fragment,
                .code = FRAGMENT_SHADER,
                .code_size = static_cast<GLint>(std::strlen(FRAGMENT_SHADER))};
        ...
        Pipeline blit_pipeline;
        {
            const ShaderInfo vertex_shader_info = {
                .stage = ShaderStage::Vertex,
                .code = BLIT_VERTEX_SHADER,
                .code_size = static_cast<GLint>(std::strlen(BLIT_VERTEX_SHADER))};
            Shader vertex_shader(ctx, vertex_shader_info);

            const ShaderInfo fragment_shader_info = {
                .stage = ShaderStage::Fragment,
                .code = BLIT_FRAGMENT_SHADER,
                .code_size = static_cast<GLint>(std::strlen(BLIT_FRAGMENT_SHADER))};
            Shader fragment_shader(ctx, fragment_shader_info);
        ...
        Pipeline cubemap_pipeline;
        {
            const ShaderInfo vertex_shader_info = {
                ...
                .code_size = static_cast<GLint>(std::strlen(CUBEMAP_VERTEX_SHADER))};
            Shader vertex_shader(ctx, vertex_shader_info);

            const ShaderInfo fragment_shader_info = {
                .stage = ShaderStage::Fragment,
                .code = CUBEMAP_FRAGMENT_SHADER,
                .code_size = static_cast<GLint>(std::strlen(CUBEMAP_FRAGMENT_SHADER))};
            Shader fragment_shader(ctx, fragment_shader_info);
