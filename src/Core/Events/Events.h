...
namespace Core
{
    ...
    concept EventHandlerFunc = requires(F&& func, const E& event) {
    ...
    namespace Detail
    {
        ...
        template<typename E, EventHandlerFunc<E> F>
        EventHandlerResult EventHandlerCallerWrapper(void* memory, const void* event)
        {
            return (*reinterpret_cast<F*>(memory))(*static_cast<const E*>(event));
        }

        using EventHandlerCallerWrapperType = EventHandlerResult (*)(void* memory,
                                                                     const void* event);

        template<typename E, EventHandlerFunc<E> F>
        void EventHandlerDeleterWrapper(void* memory) noexcept
        {
    ...
    public:
        ...
        EventHandlerRef(Detail::EventHandler* _handler) noexcept;
    ...
    class CORE_API EventListener
    {
        friend class EventEmitter;
    public:
        CORE_NON_COPYABLE(EventListener)
    ...
    private:
        std::unique_ptr<Detail::ListenerList> list;
    };

    class CORE_API EventEmitter
    {
    public:
        CORE_NON_COPYABLE(EventEmitter)
        ...
        {
            Detail::EventHandlerCallerWrapperType caller = Detail::EventHandlerCallerWrapper<E, F>;
            Detail::EventHandlerDeleterWrapperType deleter =
                Detail::EventHandlerDeleterWrapper<E, F>;
    ...
    private:
        std::map<Core::ClassIDBase::ClassIDType, Detail::EmitterList> mapping;
    };
