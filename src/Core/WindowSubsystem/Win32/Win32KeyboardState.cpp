...
namespace Core
{
    ...
    static ModifierKeyFlags
    GetModifierFlags(const std::array<BYTE, 256>& vk_keyboard_state) noexcept
    {
        constexpr static std::pair<int, ModifierKeyFlags> VK_MODS[] = {
    ...
    static std::optional<LeftRightVirtualKey> GetLeftRightVirtualKey(int vk, int scancode) noexcept
    {
        ...
        switch(vk)
        {
            ...
            case VK_MENU:
                ...
                break;
            case VK_CONTROL:
                out = LeftRightVirtualKey{.common = VK_CONTROL,
                                          .current = current,
                                          .contra = static_cast<std::uint8_t>((
    ...
    LRESULT CALLBACK Win32KeyboardState::RawInputWindowProc(HWND handle,
                                                            UINT message,
                                                            WPARAM w_param,
                                                            LPARAM l_param)
    {
        ...
        switch(message)
        {
            case WM_CREATE:
            {
                Win32KeyboardState* state = reinterpret_cast<Win32KeyboardState*>(
                    reinterpret_cast<CREATESTRUCTW*>(l_param));

                result = state->HandleWMCreate(handle);
            }
            break;
            case WM_INPUT:
            {
                Win32KeyboardState* state =
                    reinterpret_cast<Win32KeyboardState*>(GetWindowLongPtrW(handle, GWLP_USERDATA));

                result = state->HandleWMInput(reinterpret_cast<HRAWINPUT>(l_param));
            }
            break;
            default:
...
#error ADD THIS PTRS!!!!!!!!!!!!
    ...
    Win32KeyboardState::Win32KeyboardState(Win32WindowSubsystemConnection* _parent,
        ...
        : parent(_parent),
          ...
          vk_keyboard_state({}),
          scancode_to_key_mapping(),
          key_to_scancode_mapping(),
          ...
          prev_scancode(0)
    {
        ...
        WNDCLASSEXW wnd_class = {.cbSize = sizeof(WNDCLASSEXW),
                                 .style = 0,
                                 .lpfnWndProc = Win32KeyboardState::RawInputWindowProc,
                                 .cbClsExtra = 0,
                                 .cbWndExtra = 0,
                                 .hInstance = instance,
                                 .hIcon = nullptr,
    ...
    LRESULT Win32KeyboardState::HandleWMCreate(HWND handle)
    {
        Core::System::SetLastError(ERROR_SUCCESS);
        auto res = SetWindowLongPtrW(handle, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(this));
    ...
    LRESULT Win32KeyboardState::HandleWMInput(HRAWINPUT raw_input_ptr)
    {
        /*
        ...
        */
        ...
        if(GetRawInputData(raw_input_ptr,
                           RID_INPUT,
                           &raw_input,
                           &raw_input_size,
                           sizeof(raw_input.header)) != (UINT)-1)
        {
            if(raw_input.header.dwType == RIM_TYPEKEYBOARD)
            {
                ...
                //Pause = E1'1D 45
                if(stored_prev_scancode == 0xE1'1D && scancode == 0x45) //check Pause
                    scancode = 0xE1'1D'45;
                ...
                if(auto it = STABLE_SCANCODES.find(scancode); it != STABLE_SCANCODES.end())
                {
                    key = it->second;
                }
                else
                {
                    key = GetKeyByScancode(scancode);
                }
                ...
                if(current_focus)
                    window =
                        reinterpret_cast<Window*>(GetWindowLongPtrW(current_focus, GWLP_USERDATA));
                ...
                if(is_pressed)
                {
                    ...
                    if(res > 0) //neither dead key nor invalid combination
                    {
                        ...
                        {
                            utf32 =
                                ((wbuffer[0] - 0xD8'00) << 10) + (wbuffer[1] - 0xDC'00) + 0x1'00'00;
                        }
                        else
                        {
                            utf32 = wbuffer[0];
